...
class CardManager;
/*
负责协调GameModel与GameView/CardView的交互，
处理主牌堆卡牌点击匹配逻辑（selectCardFromPlayefieldAndMatch），验证卡牌是否符合消除规则（）；
管理Stack区卡牌点击事件，通过UndoManager记录操作状态（）；
实现回退功能（undo），基于UndoModel记录的状态逆向恢复卡牌位置和状态（）；
协调视图层更新，如调用CardView动画接口执行卡牌移动（）。
 */
